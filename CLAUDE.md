...
3. **Test-first.** A case lands in [docs/test-plan.md](docs/test-plan.md), then the test, then the code — the umbrella-wide order set out in [test-first-process.md](../../design/test-first-process.md). The plan is a commitment, not a wishlist: a case in the table is a case this library will cover. The coverage trail is checked both ways, so a case naming a test that does not exist, a test no case names, and a case still carrying an unresolved `[TBD]` are all errors. Where the thing a test calls does not exist yet, create the placeholder with the real signature and a `raise NotImplementedError` body so the test fails for the right reason.
4. **Source-agnostic core.** The base `Reader` interface is the contract. Adding a new backend implementation (S3, GitLab, an HTTP endpoint, a database, a packaged bundle) must not require changes to the interface, the consumer's call sites, or the settings-dispatch convention. The library is `Reader` plus a small set of concrete implementations that all honour the same contract.
5. **Typed errors, never silent defaults.** Every failure mode raises a specific exception (`ReaderNotFoundError`, `ReaderAuthError`, `ReaderNetworkError`, `ReaderParseError`). Consumers dispatch on type. The library never returns `None`, an empty string, or a sentinel object to indicate failure — those patterns hide bugs.
6. **No Evennia or Django coupling — deliberate divergence from the library standards.** This library is upstream of any Evennia integration. Pulling Evennia into the dependency graph here would slow tests, pull in a large transitive footprint, and make the library less reusable for non-Evennia contexts. Tests use pure stdlib `unittest` via `runtests.py`. The "evennia-" prefix is for ecosystem discoverability, not coupling.
7. **Synthetic content first.** Build the library against synthetic test fixtures the library owns (fixture YAML files in the test tree, fake HTTP responses, simulated filesystem layouts), exhaustively, before any consumer-library integration. Real consumer content surfaces edge cases synthetic fixtures didn't reach; when it does, pause integration, capture the case as a new synthetic fixture, fix against it, resume. Fixtures stay forever as regression coverage.
...
- **Behavioural change starts in the test plan** — principle 3. Add or amend the case, fill the **Test function** column when the test lands, then implement. The column is a coverage claim; never leave it stale.
